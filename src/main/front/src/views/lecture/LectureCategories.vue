        ...
        <div class="p-5 w-100 d-flex"> <!-- 좌측 네비, 커리큘럼 컨테이너 -->
            <div class="w-25 ps-5 me-5"> <!-- 네비 컨테이너 -->
                ...
                <ul class="nav flex-column w-100">
                    ...
                    <span>
                        <a class="fs-5 text-body text-decoration-none d-flex" data-bs-toggle="collapse" :href='"#top-"+top.top_cate'>
                            <p>{{ top.top_cate_kor }}</p>
                            <p class="ms-auto">
                                <font-awesome-icon :icon="['fas','caret-down']" />
                            </p>
                        </a>
                    </span>
                        <div class="collapse text-secondary text-secondary mid-cate-container" :id='"top-"+top.top_cate'>
                            <ul class="ps-2">
                                <li v-for="(mid, index) in top.child" :key="index" class="list-unstyled py-2">
                                    <a @click="midCategoryChanger(top,mid)" 
                                    :class="mid.mid_cate == cnt_mid_cate ? 'cnt_selected':'text-body'"
            ...
            <div class="w-75 pe-5"><!-- 강의 기본정보 & 커리큘럼 컨테이너-->
                ...
                <ul class="list-unstyled">
                    ...
                    >
                    <div>
                        <a class="d-flex text-body text-decoration-none my-auto fs-4" data-bs-toggle="collapse" :href='"#bot-cate-list-"+index'>
                            <span class="bot-cate-step">{{ item.bot_cate }}</span>  
                            <div class="bot-cate-split"></div>
                            <span class="text-body">{{ item.bot_cate_kor }}</span> 
                            <span class="ms-auto">
                                <font-awesome-icon :icon="['fas','caret-down']" />
                            </span>
                        </a>
...
export default{
    ...
    methods: {
        ...
        midCategoryChanger(top,mid){
            this.cnt_mid_cate = mid.mid_cate;
            this.cnt_mid_cate_kor = mid.mid_cate_kor;
            this.cnt_top_cate = top.top_cate;
            this.cnt_top_cate_kor = top.top_cate_kor;
        },
        /* 강의 정보 조회 */
        getLectures(top, mid){
            this.$axios.get('/api/',{
        ...
        getCurrCurriculum() {
            const curriculum = this.categories.find((category) => category.top_cate === this.cnt_top_cate)
            ?.child.find((child) => child.mid_cate === this.cnt_mid_cate)?.child;
            return curriculum || [];
        },
    },
    created() {
    ...
    mounted() {
        this.cnt_top_cate = this.categories[0].top_cate;
        this.cnt_top_cate_kor = this.categories[0].top_cate_kor;
        this.cnt_mid_cate = this.categories[0].child[0].mid_cate;
        this.cnt_mid_cate_kor = this.categories[0].child[0].mid_cate_kor;
    },
}
