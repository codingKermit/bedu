<template>
  <p id="Cschead">고객센터</p>
    <div id="csccontainer">
        <div id="csccontent">
        <table id="cscaccordion-table">
          <thead>
            <tr>
              <div>
                <b-nav tabs id="cscmy-nav">
                  <b-nav-item v-for="(item, index) in navItems" :key="index" id="cscnav-item">
                    <span class="cscitem-text">{{ item }}</span>
                  </b-nav-item>
                </b-nav>
                <div class="cscline"></div>
              </div> 
            </tr>
          </thead>
          <tbody>
            <tr v-for="(group, groupIndex) in accordionItems" :key="groupIndex">
              <td>
                <div id="cscquestion" @click="toggleAccordion(groupIndex)">
                  <span id="cscquestion-text">{{ group[0].question }}</span>
                  <i :id="['arrow-icon', { 'down': isAccordionOpen(groupIndex) }]"></i>
                </div>
                <div v-if="isAccordionOpen(groupIndex)" id="cscanswer">
                  <div v-for="(item, itemIndex) in group" :key="itemIndex">
                    <div id="cscanswer">{{ item.answer }}</div>
                  </div>
                </div>
              </td>
            </tr>
